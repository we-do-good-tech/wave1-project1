...
class PopUpGame extends Component {
  ...
  getAnsweres = () => {
    ...
    if (level[active].Kind === 1) {
      level[active].Answers.forEach((Answer, i) => {
        if (i === this.state.answered) {
          tempArr.push(
            <p className="Answer CsnAnswer" onClick={() => this.handleClick(i)}>
              {i + 1}. {Answer}
            </p>,
          );
        } else {
          tempArr.push(
            <p className="Answer" onClick={() => this.handleClick(i)}>
              {i + 1}. {Answer}
            </p>,
          );
        }
    ...
    if (level[active].Kind === 2) {
      tempArr.push(
        <p className="BtnAnswerWrong" onClick={() => this.handleClick(0)}>
          {level[active].Answers[0]}
        </p>,
      );
      tempArr.push(
        <p className="BtnAnswerRight" onClick={() => this.handleClick(1)}>
          {level[active].Answers[1]}
        </p>,
      );
      return (
    ...
    if (level[active].Kind === 3) {
      tempArr.push(
        <img
          className="PictureAnswer"
          onClick={() => this.handleClick(0)}
          src={level[active].Answers[0]}
        />,
      );
      tempArr.push(
        <img
      ...
      return (
        <div className="PicutreAnswersDiv" style={{ opacity: this.getOpacity() }}>
          ...
          <div className="PicutreAnswersDivBtns">{tempArr}</div>
        </div>
      );
    }
  ...
  getQuestion = () => {
    const level = this.state.level[this.state.active];
    return (
      <p id="Question" style={{ opacity: this.getOpacity() }}>
        {this.state.active + 1}. {level.Question}{' '}
      </p>
    );
  };
  ...
  checkAnswer = (i) => {
    const { level } = this.state;
    const { active } = this.state;
    if (level[active].right === i) return true;
  };
  ...
  handleClick = (myanswer) => {
    ...
    if (this.state.clickable) {
      ...
      if (level[active].Kind === 4) {
        right += 1;
        this.setState({ clickable: false, display: 'block', buttontype: 1, tries: 0, right });
    ...
    if (this.state.buttontype === 1) {
      return (
        <div style={{ display: this.state.display }} id="explanationPopUp">
          ...
          {this.isLast() ? (
            <button className="NextButton" onClick={() => this.handleNextButton()}>
              לסיום המבחן
            </button>
          ) : (
            <button className="NextButton" onClick={() => this.handleNextButton()}>
      ...
      return (
        <div style={{ display: this.state.display }} id="explanationPopUp">
          <button
            ...
            onClick={() => this.setState({ clickable: true, display: 'none', answered: '' })}
          >
            נסו שוב
          </button>
          <p id="V" style={{ 'background-color': '#DC5B59' }}>
            ✖
          </p>
  ...
  getScore = () => {
      ...
      return (
        <div id="score">
          ...
          <div id="ScoreBtnsDiv">
            <button
              onClick={() => {
                this.setState({ active: 0, right: 0, LastQuestion: false });
              }}
            >
              התחל שוב{' '}
            </button>
    ...
    if (this.state.right >= this.state.level.length * 0.8) {
      return (
        <div id="score">
          <p> כל הכבוד !</p>
          <p>
          ...
          <div id="ScoreBtnsDiv">
            <button
              onClick={() => {
                this.setState({ active: 0, right: 0 });
              }}
            >
              התחל שוב{' '}
            </button>
    ...
    if (this.state.right >= this.state.level.length * 0.4) {
      return (
        <div id="score">
          <p> כל הכבוד !</p>
          <p>
    ...
    if (this.state.right >= this.state.level.length * 0.1) {
      return (
        <div id="score">
          <p> כל הכבוד !</p>
          <p>
    ...
    if (this.state.level[this.state.active].Kind === 1) {
      return (
        <img
          ...
          src={this.state.level[this.state.active].img}
  ...
  getOpacity = () => {
    if (this.state.clickable) return 1;
    return 0.2;
  };

  skip = () => {
    const { clickable } = this.state;
    if (clickable) this.setState({ active: parseInt(this.state.active) + 1 });
  };

  isLast = () => {
    if (this.state.active === this.state.level.length - 1) return true;
    return false;
  };
  ...
  render() {
    if (this.state.active != this.state.level.length) {
      return (
        <div className="PopUpGameDiv">
          <p className="PopUpGameHeadLine" style={{ opacity: this.getOpacity() }}>
            {' '}
            בחן את עצמך בנושא:
            {this.state.HeadLine}
          </p>
          <p
            className="PopUpGameHeadLine"
          ...
          <button
            id="Skip"
            className={!this.state.clickable && 'SkipDisabled'}
            onClick={() => this.skip()}
          >
            דלג
          </button>
    ...
    return <div className="PopUpGameDiv">{this.getScore()}</div>;
  }
}
