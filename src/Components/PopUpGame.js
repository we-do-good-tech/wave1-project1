...
class PopUpGame extends Component {
	...
	getAnsweres = ()=>{
		...
		if(level[active].Kind===1){
			level[active].Answers.forEach((Answer,i)=>{
				...
				{
					tempArr.push(<p className="Answer CsnAnswer" onClick={()=>this.handleClick(i)}>
  {i+1}
  . 
  {' '}
  {Answer}
				...
				{
				tempArr.push(<p className="Answer" onClick={()=>this.handleClick(i)}>
  {i+1}
  . 
  {' '}
  {Answer}
  ...
  <div className="PopUpGameDiv">
    ...
    <p className="PopUpGameHeadLine" style={{"margin-top":'-1.2vw', 'font-size':'1.6vw',opacity:this.getOpacity()}}>
      {' '}
      שאלה
      {this.state.active+1}
      {' '}
