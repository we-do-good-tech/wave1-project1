...
class WhatIsBoccia extends Component {
  ...
  render() {
        ...
        return (
          <div className="ContentDiv">
            ...
            <p className="ContentText" style={{ width: '42vw' }}>
              בוצ׳ה (הגייה - BO-CHA) הינו ענף פראלימפי המשלב מיומנויות שונות. מטרת המשחק היא לקרב
              כמה שיותר כדורים אל כדור המטרה. <br />
              השם ”בוצ'ה“ נגזר מהמילה הלטינית ”כדור“.
            </p>{' '}
            <p className="ContentText" style={{ width: '42vw' }}>
              בוצ'ה הוא ענף המשלב שליטה, דיוק, חשיבה ואסטרטגיה. המשחקים נמשכים ארבעה או שישה
              משחקונים (בבוצ'ה מערכה נקראת "משחקון"). השחקנים זורקים או מגלגלים כדורים לעבר כדור
              המטרה הלבן. <br />
              לכל צד יש שישה כדורים (אדום או כחול), ובכל משחקון, מנסים לצבור כמה שיותר נקודות על ידי
              זריקת ששת הכדורים והבאתם קרוב ככל הניתן לכדור המטרה. אם בתום כל המשחקונים, המשחק
              מסתיים בשיוויון, משחקים שובר שוויון. ניתן לשחק בוצ'ה אחד על אחד (יחידים),
              <br /> שניים מול שניים (זוגות), או שלושה מול שלושה (קבוצתי).
            </p>
            <img
              className="ContentPic"
              src={WhatIsOne}
              style={{ width: '28vw', left: '3vw', top: 0 }}
            />
          </div>
        );
      }
        ...
        return (
          <div className="ContentDiv">
            ...
            <p className="ContentText">
              בוצ׳ה הוא ענף ספורט המיועד לנשים וגברים בכל הגילאים להם פגיעה בארבע גפיים. המשחק יועד
              בתחילה לספורטאים עם שיתוק מוחין (Cerebral Palsy), עם השנים התרחב גם לספורטאים עם
              ליקויים אחרים, כגון: ניוון שרירים, פוליו, פגיעות ראש ועוד.
            </p>
            <p className="ContentText" style={{ width: '42vw' }}>
              השחקנים במשחק משתייכים ל-4 קבוצות סיווג (קלאסים):
              <br />
              BC1, BC2, BC3, BC4. כל סיווג מתייחס לרמת תפקוד שונה.
            </p>
            <img
              className="ContentPic"
              src={WhatIsSecond}
              style={{ width: '36vw', top: '6.5vw', left: '6vw' }}
            />
          </div>
        );
      }
    ...
    const GetDivFrontBack = () => {
      if (this.state.active === this.state.names.length) {
        return <FrontBack moveTo={1} last onClick={this.handlearrow} />;
      }
      if (this.state.active === 1) {
        return <FrontBack moveTo={1} first onClick={this.handlearrow} />;
      }

      if (this.state.active != 0) {
        return <FrontBack moveTo={1} onClick={this.handlearrow} />;
      }
    };
