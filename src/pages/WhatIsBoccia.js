...
class WhatIsBoccia extends Component {
	...
	render(){
		...
		let GetDivContent= () => {			
				...
				return(
					<div className="ContentDiv">
								...
								בוצ'ה הוא ענף המשלב שליטה, דיוק, חשיבה ואסטרטגיה. המשחקים נמשכים ארבעה או שישה משחקונים (בבוצ'ה מערכה נקראת "משחקון").
								 השחקנים זורקים או מגלגלים כדורים לעבר כדור המטרה הלבן. <br/>
								 ...
								 אם בתום כל המשחקונים, המשחק מסתיים בשיוויון, משחקים שובר שוויון. ניתן לשחק בוצ'ה אחד על אחד (יחידים),<br/> זוג מול זוג (זוגות), או  שלושה מול שלושה (קבוצתי).
								</p>
			...
			if(this.state.active==2)
			{
				return(
					<div className="ContentDiv">
								...
								<img className="ContentPic" src={WhatIsSecond} style={{width:'32vw',top:'6.5vw',left:'6vw'}}/>						
					</div>
				)
			}
			...
			if(this.state.active===this.state.names.length){
				return(
					<FrontBack moveTo={1} last={true} onClick={this.handlearrow}/>
				);
			}
			if(this.state.active===1){
				return(
